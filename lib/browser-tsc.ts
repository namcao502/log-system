...
function getCellForDate(date: Date, emit: (line: string) => void): string {
  ...
  emit(`[tsc-log] Date: ${year}-${month}-${day}, dayOfYear: ${dayOfYear}, row: ${row}`);
...
async function getExcelFrame(page: Page, emit: (line: string) => void): Promise<Frame> {
  ...
  for (const selector of iframeSelectors) {
    try {
      ...
      if (iframeEl) {
        ...
        if (frame) {
          emit(`[tsc-log] Found Excel iframe via: ${selector}`);
          return frame;
        }
      }
    } catch {
  ...
  emit("[tsc-log] No iframe found, using main page frame");
...
async function navigateToCell(
  ...
  try {
    ...
    emit("[tsc-log] Name Box not found");
    return false;
  }

  for (let attempt = 1; attempt <= 3; attempt++) {
    emit(`[tsc-log] Nav attempt ${attempt} → ${cellAddress}`);
    ...
    emit(`[tsc-log] Name Box after nav: "${nbValue}"`);

    if (nbValue === cellAddress.toUpperCase()) {
      emit(`[tsc-log] Navigated to ${cellAddress}`);
      return true;
    }

    emit(`[tsc-log] Expected "${cellAddress}", got "${nbValue}" — retrying`);
    await page.waitForTimeout(500);
  }

  emit(`[tsc-log] Nav verification failed after 3 attempts`);
  return false;
}
...
async function attemptWrite(
  ...
  try {
    ...
    emit(`[tsc-log] [${elapsed()}] Browser launched`);

    const page = context.pages()[0] || (await context.newPage());

    await page.goto(EXCEL_URL, { waitUntil: "domcontentloaded", timeout: 60000 });
    emit(`[tsc-log] [${elapsed()}] DOM loaded`);

    const frame = await getExcelFrame(page, emit);
    emit(`[tsc-log] [${elapsed()}] Iframe found`);

    try {
      await frame.waitForSelector("[id*='NameBox'] input", { timeout: 30000 });
      emit(`[tsc-log] [${elapsed()}] Name Box visible`);
      await frame.waitForSelector("[id*='formulaBar'] [contenteditable]", { timeout: 10000 });
      emit(`[tsc-log] [${elapsed()}] Formula bar visible`);
    } catch {
      emit(`[tsc-log] [${elapsed()}] Excel UI not fully detected, continuing...`);
    }
    await page.waitForTimeout(3000);
    emit(`[tsc-log] [${elapsed()}] Excel ready`);
    ...
    for (const cellAddress of cellAddresses) {
      ...
      emit(`[tsc-log] [${elapsed()}] Nav done → ${cellAddress}`);
      if (!navSuccess) {
        await context.close();
      ...
      emit(`[tsc-log] [${elapsed()}] ${cellAddress} value: "${currentValue}"`);

      if (currentValue.includes(ticket)) {
        emit(`[tsc-log] [${elapsed()}] "${ticket}" already in ${cellAddress}, skipping`);
      ...
      await page.waitForTimeout(500);
      emit(`[tsc-log] [${elapsed()}] Typed "${newValue}" into ${cellAddress}`);

      const fbAfter = (await formulaBar.textContent())?.trim() ?? "";
      emit(`[tsc-log] Formula bar after typing: "${fbAfter}"`);

      await formulaBar.press("Enter");
      emit(`[tsc-log] [${elapsed()}] Committed ${cellAddress}`);
    ...
    await page.waitForTimeout(3000);
    emit(`[tsc-log] [${elapsed()}] Done!`);
    await context.close();

    return { success: true, cell: cellsLabel };
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    emit(`[tsc-log] [${elapsed()}] Error: ${message}`);
    if (context) {
...
export async function writeTicketViaPlaywright(
  ...
  emit(`[tsc-log] Target cells: ${cellAddresses.join(", ")}, ticket: ${ticket}`);

  const errors: string[] = [];

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    if (attempt > 1) {
      emit(`[tsc-log] Retry attempt ${attempt}/${MAX_ATTEMPTS}`);
    }
    ...
    emit(`[tsc-log] Attempt ${attempt} failed: ${msg}`);
  }
